...
namespace {
	...
	FlutterTtsPlugin::FlutterTtsPlugin(
		...
		: registrar_(registrar),
		  window_handle_(registrar != nullptr && registrar->GetView() != nullptr
	...
	void FlutterTtsPlugin::PostToPlatformThread(
		...
		PostMessage(window_handle_, kDispatchCallbacksMessage, 0, 0);
	}

	std::optional<LRESULT> FlutterTtsPlugin::HandleWindowMessage(
		HWND hwnd, UINT message) {
		if (hwnd != window_handle_ || message != kDispatchCallbacksMessage) {
			return std::nullopt;
		}
