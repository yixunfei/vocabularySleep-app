...
## 基本信息
- **创建日期**: 2026-08-25
- **状态**: 进行中（阶段 1-16 已完成，阶段 17 进行第二轮剩余风险审计）
- **负责人**: Codex
...
17. **阶段 16：连续播放日志背压**。限制调试模式播放/TTS 高频日志的内存排队量，改为有界批量落盘并避免逐条强制 flush；验证长时间播放后日志队列、UI 事件循环与日志恢复语义。
18. **阶段 17：第二轮剩余风险审计**。复核跨模块全局监听、搜索快照常驻、同步查询回退、定时器/订阅释放和大词表派生集合，按真实 12000 词基准决定下一项优化，未达到量化阈值的候选只记录不改动。
...
- **风险 9**: 调试模式连续播放会高频生成播放/TTS 日志，逐条 `flush: true` 的无界 Future 链在慢存储上可能积压并拖慢其他模块。缓解：阶段 16 引入有界缓冲、单写入循环和批量追加，同时保留错误日志与测试可等待的 flush 边界。
- **风险 10**: 完整搜索快照仍可能持有大量 lite 词条，且 `%query%` 无法使用普通 B-tree；超大词本或宽泛查询可能把后台 CPU 成本转化为 heap/GC 压力。缓解：阶段 17 量化快照大小、生命周期和查询计划，再决定分页 ID 快照或 FTS5/trigram 索引方案。
...
- 第二轮不把全部 `AppState` 消费者一次性迁移为细粒度 provider；仅处理本模块已确认的高频广播源，并把全应用状态拆分列为独立架构候选。
...
## 阶段 15 执行结果
- 大词本搜索 SQL、计数和 lite `WordEntry` 物化全部在只读 SQLite worker 内完成；Library、Play、Practice 共用带签名的完成快照，搜索进行中不再同步回退数据库查询。
- 搜索状态使用 generation、词本 ID、查询和模式签名校验；快速输入只允许一个 worker 运行，中间请求被最新待执行请求覆盖，避免低端设备同时执行多次全表扫描并堆叠结果对象。
- 清空查询、切换词本、数据库恢复和 `dispose` 会清空待执行请求与已提交快照；迟到结果不能回写，Library/Play/Practice 分别显示独立加载态。
- 真实 12000 词基准：`a` 的同步全量路径约 215ms、worker 约 208-216ms；`ab` 约 104ms/109ms；`ability` 约 67ms/71ms。worker 期间 10ms UI 定时器最大间隔约 11.0-12.5ms，说明总 CPU 近似不变但 UI isolate 不再冻结。
- 原始 worker 按 160ms 间隔启动三次搜索时总计约 485ms、UI 最大间隔约 12.0ms；产品层进一步串行合并请求并由回归确认最大并发数为 1。前缀跳转仅约 0-4ms，不作为本轮优化项。
- 验证覆盖 worker 查询、分页快照、快速输入、切词本、清空快照、`dispose` 和三个页面加载态；核心回归 50 项、目标 UI smoke 5 项通过，目标分析无 error。本阶段新增/退休 i18n key 均为 0，未触达 catalog。

## 阶段 16 执行结果
- `AppLogService` 将 DEBUG 日志限制为仅 debug 构建控制台输出，发布构建在 payload/JSON 构造前直接返回；INFO/WARN/ERROR 统一进入单一有界文件写入器。
- 文件队列最多保留 1024 行和 1 MiB 字符，单批最多 128 行/256 KiB；背压按 INFO、WARN、ERROR 优先级淘汰，超大单行先截断为有效 JSON，避免慢存储形成无界 Future/字符串链。
- 文件追加由逐条 `flush: true` 改为 `flush: false` 批量写入；写入失败仍执行目录/文件重建和一次重试，path-provider 不可用仍退化为控制台日志。
- `reset` 使用 generation 丢弃旧批次，并等待活跃 drain 退出后再启动新代际，避免测试重置时并发追加或旧统计污染。
- 2000 条 INFO（每条约 160 字符 payload）同机基准：旧实现入队约 25ms、flush 约 2937ms、入队后 RSS 增量约 5.7MB；新实现入队约 30ms、flush 约 10ms、RSS 增量约 0.7MB。新实现 10ms timer 最大间隔约 32.3ms，主要发生在同步构造 2000 条 JSON 的突发入队阶段，因此日志调用点节流/惰性 data 构造仍列为后续候选。
- 验证：日志服务 5 项（含删除目录恢复、console-only 回退、DEBUG 不落盘、5000 条背压和 reset 竞态）、播放/TTS/AppState 初始化 35 项通过；目标 `flutter analyze` 无问题，`dart format` 通过。本阶段新增/退休 i18n key 均为 0，未触达 catalog。
