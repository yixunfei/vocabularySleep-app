...
### 原因
- 按二次审查结果继续降低睡眠支持在夜间收尾、重复进入和跨模块提醒场景中的操作负担。

### 修改
- 夜间支持进入休息时写入可重试检查点；跨自然日自动开启新会话，保存失败不再阻挡新的即时引导。
- 夜间动作隐藏重复的“换方法/休息”出口，重复点击正在播放的声音不再重启计时；结束支持会停止睡眠模块启动的声音。
- 睡眠提醒按需初始化专注服务并检查创建结果，睡眠类提醒点击后进入工具箱入口；白天日记首个回答即可保存，评估草稿重载时保留未保存修改。

### 新增
- 新增睡眠会话连续性回归测试，补充提醒不可用与权限未开启的本地化文案。

### 验证
- 睡眠会话、连续性与夜间窄屏 Widget 测试全部通过（20 项）。
- 目标文件 `dart analyze` 无 error；仅保留既有 `focus_service.dart` 风格提示。

### 风险变更
- 系统提醒权限未开启时仍保存应用内提醒，但页面明确提示系统通知不可用；真机通知、锁屏音频和冷启动耗时仍需设备验收。

## [Unreleased-PLAN_434-PROJECT-COMPREHENSIVE-AUDIT] - 2026-09-10

### 原因
- 按要求对当前项目进行用户体验、性能、稳定性、功能扩展、包体大小、安全边界和工程治理的全面只读审计。

### 新增
- 新增专业审计报告：`records/record_314_project_comprehensive_audit_20260910.md`。
- 新增审计计划：`plans/PLAN_434_项目全面审计与治理建议.md`。
- 报告覆盖 P0/P1/P2/P3 风险、证据路径、分阶段治理路线、测试缺口和发布质量门禁。

### 修改
- 无业务代码、配置、数据库、依赖或线上资源修改。

### 验证
- 复核已有 `flutter analyze --no-pub` 与 `flutter test --no-pub --reporter compact` 审计结果：未发现 error；测试未见失败摘要。
- 复核 Android APK/AAB、资源清单、依赖配置、设计规范和国际化审计文档。
- 工作范围保持只读审计；未执行凭据轮换、依赖升级或重新打包。

### 风险变更
- 报告确认客户端 S3 凭据、API key 保护、零时长睡眠步骤、缓存路径校验、包体和启动内存等高优先级风险，均留待后续独立修复计划处理。

## [Unreleased-PLAN_432-SLEEP-SUPPORT-REDESIGN] - 2026-09-09

### 原因
- 夜间入口被评估、长表单和多项设置占据，对疲惫、夜醒和烦躁用户操作负担过重；原有页面计时、快捷预填和按记录数量截取报告存在功能与数据问题。

### 新增
- 首页提供准备睡觉、半夜醒来、烦躁难受三条直接路径；逐步点击引导，可跳过、换方法、安静休息和结束，无必填文本、评估门槛或夜间倒计时。
- 独立支持会话、习惯计时和声音控制器。声音按需离线生成，默认低音量，支持 15 / 30 / 60 分钟停止、失败重试和后台返回校正。
- 白天集中提供分步日记、可选评估、真实自然日报告、夜间支持历史、每天一个行动的多日计划，以及原有模板、思绪、提醒和科普入口。

### 修复
- 页面切换不再终止习惯计时，后台返回按实际经过时间推进；秒级计时及评估草稿输入不再触发全局 AppState 刷新。
- 未记录的时长、次数、环境因素保持空值；快捷修改保留旧记录中未改字段，详细表单同步快捷答案，备注标签正确追加。
- 报告覆盖最近 7 / 14 个自然日，排除未来记录，缺测不连线，显示各指标有效样本；多日计划禁止提前、补刷及重复完成。
- 夜间结束防止重复保存与重复返回，失败可重试且沿用会话 ID；首次直接进入夜间流程后保存会先保留存储中的历史。离床及回床仅记录用户明确确认，不推断入睡。
- 日记、评估保存失败保留输入；夜间历史每条记录使用独立展开状态。声音在真正准备成功后显示播放，取消和切换后的迟到回调不再覆盖当前状态。
- Git 忽略规则明确放行 Dart 领域模型源码，避免新增会话模型被下载模型资源的忽略规则误排除。

### 验证
- 全量 Flutter 884 项测试通过；随后补充的声音切换截止时间回归及声音专项 9 项通过，最终睡眠专项 71 项通过。睡眠专项覆盖真实 AppState/仓库、快速连点、失败重试、时间跳跃、自然日统计、详细字段同步及窄屏流程。
- 全仓 `dart analyze` 无 error，保留 142 条既有 warning/info；Windows debug 原生构建成功（63.1 秒），输出 `build/windows/x64/runner/Debug/xianyushengxi.exe`。
- 375×667 中文页面已渲染检查；320 / 375 / 600dp 与 1.3 倍字体、德语/俄语长译文已做 Widget 回归。
- i18n 新增 146 key、退休 0 key，七语言 CSV 和 registry 同步；审计 53346 keys，缺失、占位符和 params 错误为 0，旧 helper 与 Dart 插值扫描无命中。
- 维护报告：40683 个潜在未引用 key、2002 个 stale source、160 个 ready retirements。未引用增加来自旧页面移除与动态 key 调用，不在功能提交中做历史 catalog 瘦身。
- 验证命令：`flutter test --no-pub --reporter expanded`、目标 Dart 格式化/分析、`node scripts/audit_i18n_placeholders.js`、`node scripts/maintain_i18n_catalog.js --limit 20` 及 AGENTS.md 规定的两项 rg 扫描。

### 风险变更
- SleepDailyLog 未记录数值/布尔值直接采用 nullable 格式，不引入兼容层；保留历史明确存储值，无法识别并纠正旧快捷预设曾生成的虚构数据。
- 会话与习惯计时恢复覆盖同一进程中的页面切换和生命周期；不保证进程被系统终止后的恢复。Android/iOS 锁屏音频、系统息屏和低端设备帧耗时仍需真机验证。
- 新增模块按职责拆分；历史 AppState 和通用 ui_smoke_test 超长文件仅做必要接入，未在本轮全面重构。沿用现有 toolbox 视觉基线，无新增运行时依赖、无发行版本变更。
- 原工作区已按 PLAN_431 合并本地 main；本次重设计在 `codex/sleep-support-redesign` 单独提交，不推送或发布远端。
...
### 原因
- 需要验证尺八类乐器是否可以不用麦克风吹气检测，而改用触控气流面板表达吹气开始、强弱变化和停止。
- 当前工作树存在其他模块并行改动，本轮需要控制影响面积，避免混入无关文件。

- 全屏尺八需要改为现代五孔布局（前四后一）和两列操作区，同时移除重复全屏入口与“测试用乐器”描述。

### 新增
- 新增尺八测试乐器：普通模式只显示进入全屏按钮，全屏内显示音级选择和气流控制面板。
...
- `git diff --check` 通过。
- 追加验证：`dart format lib/src/ui/pages/toolbox_sound_tools/shakuhachi.dart lib/src/ui/pages/toolbox_sound_tools/deck.dart` 通过。
...
### 原因
- 用户确认优先推进真实采样乐器替换，并要求由代理全权接管开源音色库选择、开发暂存下载和后续 S3 资源迁移前准备。

### 新增
- 新增 `flutter_midi_engine` 依赖，作为首批 SoundFont/SF3 移动端 POC 引擎。
...
- `dart analyze lib\src\services\toolbox_audio_service.dart lib\src\ui\pages\toolbox_sound_tools.dart test\toolbox_instrument_engine_test.dart` 通过，No issues found。
- `flutter test test\toolbox_instrument_engine_test.dart --reporter compact` 通过。
- `flutter test test\toolbox_audio_bank_regression_test.dart --reporter compact` 通过；运行前出现一次 Windows `build\unit_test_assets` 清理提示，但测试用例全部通过。
- `node scripts\audit_i18n_placeholders.js` 通过，missing 0，placeholderMismatch 0，Dart missingParams 0。
