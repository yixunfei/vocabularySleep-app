...
- 工具箱/生活实用隐写工具已扩展为本地加密与媒体隐写复合模块：支持文本/文件 payload 写入图片、音频、视频载体，内置 AES/Twofish/Camellia 固定与自由级联、256/512/1024-bit 每层密钥材料、Whirlpool MAC/哈希、SHA-256/RSA 与 ECDSA 验签层、口令 + keyfile 混合派生和随机 keyfile 导出；该能力只处理本地媒体载荷，不进入主数据库或学习记录。
