...
- 工具箱/生活实用隐写工具已扩展为本地加密与媒体隐写复合模块：支持文本/文件 payload 写入图片载体，内置 AES/Twofish/Camellia 固定与自由级联、256/512/1024-bit 每层密钥材料、Whirlpool MAC/哈希、SHA-256/RSA 与 ECDSA 验签层、口令 + keyfile 混合派生和随机 keyfile 导出；图片 LSB 使用密钥派生头与 CSPRNG 随机像素顺序，RC4/SHA256 stream 仅保留旧载荷还原兼容，音视频新写入等待频域/帧级后端后再启用；页面已将 keyfile 收口为开关式选项，支持多 keyfile 稳定排序组合，并折叠高级设置以降低移动端滚动长度；该能力只处理本地媒体载荷，不进入主数据库或学习记录。
