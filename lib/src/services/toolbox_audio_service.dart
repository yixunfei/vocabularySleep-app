...
class ToolboxAudioBank {
  ...
  static void _applySingingBowlSpace(
    List<double> samples, {
    required int sampleRate,
    required double amount,
  }) {
  ...
  static Uint8List _buildSingingBowlTone({
    required double frequency,
    required String style,
    required int variant,
  }) {
    ...
    for (var i = 0; i < totalSamples; i += 1) {
      final t = i / sampleRate;
      final attackProgress = (t / math.max(attackSeconds, 0.02)).clamp(
        0.0,
        1.0,
      );
      ...
      final amplitudeMod =
          (1 +
                  lfoDepth *
                      math.sin(math.pi * 2 * lfoFrequency * t + lfoPhase) +
                  0.011 *
                      math.sin(
      ...
      for (var h = 0; h < harmonics.length; h += 1) {
        ...
        final toneA = waveSample(
          waveforms[h],
          math.pi * 2 * partialFrequency * detune * drift * t +
              partialPhaseA[h],
        );
