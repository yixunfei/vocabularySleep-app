...
class DailyChoiceActivityLibraryStatus {
  const DailyChoiceActivityLibraryStatus({
  ...
  DailyChoiceActivityLibraryStatus copyWith({
    ...
    return DailyChoiceActivityLibraryStatus(
...
typedef DailyChoiceActivitySupportDirectoryProvider =
    Future<Directory> Function();

class DailyChoiceActivityLibraryStore {
  DailyChoiceActivityLibraryStore({
    CstCloudS3CompatClient? s3Client,
    DailyChoiceActivitySupportDirectoryProvider? supportDirectoryProvider,
    this.databaseFileName = 'toolbox_daily_choice_activity.db',
    this.remoteLibraryKey = 'activity_data/daily_choice_activity_library.json',
  }) : _supportDirectoryProvider =
           supportDirectoryProvider ?? getApplicationSupportDirectory,
       _s3Client = s3Client ?? CstCloudS3CompatClient(),
       _ownsS3Client = s3Client == null;

  final DailyChoiceActivitySupportDirectoryProvider _supportDirectoryProvider;
  ...
  static const String _optionsTable = 'daily_choice_activity_options';
  static const String _metaTable = 'daily_choice_activity_meta';
  static const int _schemaVersion = 1;

  Future<DailyChoiceActivityLibraryStatus> inspectStatus() async {
    final file = await _databaseFile();
    if (!await file.exists()) {
      return const DailyChoiceActivityLibraryStatus.empty();
    }
    final db = await _database();
    return _inspectOpenDatabase(db);
  }

  Future<DailyChoiceActivityLibraryStatus> installLibrary() async {
    ...
    try {
      ...
      if (!status.hasInstalledLibrary || status.actionCount <= 0) {
        throw StateError('Remote activity library is empty.');
      }
  ...
  void _ensureSchema(Database db) {
    ...
    if (currentVersion > _schemaVersion) {
      throw StateError(
        'Activity library schema version $currentVersion is newer than supported $_schemaVersion.',
      );
    }
  }
  ...
  DailyChoiceActivityLibraryStatus _inspectOpenDatabase(Database db) {
    if (!_tableExists(db, _optionsTable)) {
      return const DailyChoiceActivityLibraryStatus.empty();
    }
    ...
    final actionCount = (countRow.firstOrNull?['total'] as num?)?.toInt() ?? 0;
    if (actionCount <= 0) {
      return const DailyChoiceActivityLibraryStatus.empty();
    }
  ...
  DailyChoiceActivityLibraryStatus _inspectDatabaseFile(File file) {
    final db = sqlite3.open(file.path);
    try {
      db.execute('PRAGMA foreign_keys = ON;');
      _ensureSchema(db);
  ...
  DailyChoiceOption? _loadDetail(Database db, String optionId) {
    ...
    return _optionFromRow(rows.first);
  }
