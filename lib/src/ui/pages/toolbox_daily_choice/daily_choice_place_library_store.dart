...
class DailyChoicePlaceLibraryStatus {
  const DailyChoicePlaceLibraryStatus({
  ...
  DailyChoicePlaceLibraryStatus copyWith({
    ...
    return DailyChoicePlaceLibraryStatus(
...
typedef DailyChoicePlaceSupportDirectoryProvider = Future<Directory> Function();

class DailyChoicePlaceLibraryStore {
  DailyChoicePlaceLibraryStore({
    CstCloudS3CompatClient? s3Client,
    DailyChoicePlaceSupportDirectoryProvider? supportDirectoryProvider,
    this.databaseFileName = 'toolbox_daily_choice_place.db',
    this.remoteLibraryKey = 'place_data/daily_choice_place_library.json',
  }) : _supportDirectoryProvider =
           supportDirectoryProvider ?? getApplicationSupportDirectory,
       _s3Client = s3Client ?? CstCloudS3CompatClient(),
       _ownsS3Client = s3Client == null;

  final DailyChoicePlaceSupportDirectoryProvider _supportDirectoryProvider;
  ...
  static const String _optionsTable = 'daily_choice_place_options';
  static const String _metaTable = 'daily_choice_place_meta';
  static const int _schemaVersion = 1;

  Future<DailyChoicePlaceLibraryStatus> inspectStatus() async {
    final file = await _databaseFile();
    if (!await file.exists()) {
      return const DailyChoicePlaceLibraryStatus.empty();
    }
    final db = await _database();
    return _inspectOpenDatabase(db);
  }

  Future<DailyChoicePlaceLibraryStatus> installLibrary() async {
    ...
    try {
      ...
      if (!status.hasInstalledLibrary || status.placeCount <= 0) {
        throw StateError('Remote place library is empty.');
      }
  ...
  void _ensureSchema(Database db) {
    ...
    if (currentVersion > _schemaVersion) {
      throw StateError(
        'Place library schema version $currentVersion is newer than supported $_schemaVersion.',
      );
    }
  }
  ...
  DailyChoicePlaceLibraryStatus _inspectOpenDatabase(Database db) {
    ...
    final placeCount = (countRow.firstOrNull?['total'] as num?)?.toInt() ?? 0;
    if (placeCount <= 0) {
      return const DailyChoicePlaceLibraryStatus.empty();
    }
  ...
  DailyChoicePlaceLibraryStatus _inspectDatabaseFile(File file) {
    final db = sqlite3.open(file.path);
    try {
      db.execute('PRAGMA foreign_keys = ON;');
      _ensureSchema(db);
