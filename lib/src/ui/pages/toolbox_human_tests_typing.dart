...
class _TypingPassage {
  ...
  String text(AppI18n i18n) {
    return switch (language) {
      ...
      _TypingLanguage.mixed => pickUiText(i18n, zh: zh, en: en),
    };
  }
}
      ...
      children: <Widget>[
        _HumanMetricWrap(
          metrics: <(String, String)>[
            (pickUiText(i18n, zh: '速度', en: 'Speed'), '${liveWpm.round()} WPM'),
            (
              pickUiText(i18n, zh: '净速', en: 'Net'),
              '${liveNetWpm.round()} WPM',
            ),
            (pickUiText(i18n, zh: '字符', en: 'Chars'), '${liveCpm.round()} CPM'),
            (
              pickUiText(i18n, zh: '准确率', en: 'Accuracy'),
              input.isEmpty ? '-' : '${accuracy.round()}%',
            ),
            (pickUiText(i18n, zh: '错误', en: 'Errors'), '${stats.errors}'),
            (
              pickUiText(i18n, zh: '稳定性', en: 'Stability'),
              input.isEmpty ? '-' : '${consistency.round()}%',
            ),
            (pickUiText(i18n, zh: '回退', en: 'Backspaces'), '$_backspaces'),
            (
          ...
          child: Column(
            ...
            children: <Widget>[
              ...
              Wrap(
                ...
                children: <Widget>[
                  _HumanActionButton(
                    label: pickUiText(i18n, zh: '重新开始', en: 'Restart'),
                  ...
                  OutlinedButton.icon(
                    ...
                    label: Text(pickUiText(i18n, zh: '换一段', en: 'New passage')),
                  ),
                  OutlinedButton.icon(
                    onPressed: () => _focusNode.requestFocus(),
                    icon: const Icon(Icons.keyboard_rounded),
                    label: Text(
                      pickUiText(i18n, zh: '聚焦输入', en: 'Focus input'),
                    ),
                  ),
                ],
              ),
        ...
        const SizedBox(height: 12),
        _HumanSettingsSection(
          title: pickUiText(i18n, zh: '训练边界', en: 'Training boundary'),
          subtitle: pickUiText(
            i18n,
            zh: '模式、语言、题材和长度只影响当前页面即时训练。',
            en: 'Mode, language, topic, and length only affect this local session.',
          ),
          child: Text(
            pickUiText(
              i18n,
              zh: '结果报告不会写入学习记录。建议先用经典模式建立基准，再切换冲刺、精准、盲打、符号、代码和数字模式做专项练习。',
              en: 'Reports are not saved to study history. Start with Classic, then use Sprint, Precision, Blind, Symbols, Code, and Numbers for focused practice.',
            ),
            style: Theme.of(
              context,
